...
Offsetting gained a separate 14-case Knipsa/Clipper2 matrix covering closed
...
`0.85x` on overlapping-offset cleanup. Those measurements are a historical
baseline, not a universal speed claim.

The 0.2.1 release run on 2026-08-08 matched all 14 cases and measured Knipsa
ahead in each one, from `1.04x` through `5.78x`; overlap cleanup measured
`1.19x`. The improvement closes that small checked-in case, but larger overlap
scale curves, allocation counts, and tail latency remain open evidence work.
...
| P1 | Overlapping offset cleanup was the only 2026-08-05 offset workload behind Clipper2 (`0.85x` reference/Knipsa); the 0.2.1 release run measured `1.19x`. | Keep improving the shared exact-union broad phase rather than adding an offset-only shortcut. | Differential equality against all-pairs oracle and a larger scale curve. |
