...
pub(super) fn try_boolean_opd(request: BooleanRequestD<'_>) -> Option<PathsD> {
    if request.subjects.len() != 1
        ...
        || !matches!(
            request.fill_rule,
            FillRule::EvenOdd | FillRule::NonZero | FillRule::Positive
        )
    {
        return None;
    }
...
fn certified_positive_convex(path: &[PointD]) -> bool {
    ...
    for index in 0..path.len() {
        ...
        if robust_cross_order(incoming, outgoing) != Some(Ordering::Greater) {
            return false;
        }
        signed_area += current.x * next.y - current.y * next.x;
    }
    signed_area.is_finite() && signed_area > f64::EPSILON
}
...
fn find_two_crossings(subject: &[PointD], clip: &[PointD]) -> Option<[Crossing; 2]> {
    ...
    for _ in 0..limit {
        ...
        if let Some((point, subject_parameter, clip_parameter)) = segment_intersection(
            ...
            if subject_parameter <= PREDICATE_TOLERANCE
                || subject_parameter >= 1.0 - PREDICATE_TOLERANCE
                || clip_parameter <= PREDICATE_TOLERANCE
                || clip_parameter >= 1.0 - PREDICATE_TOLERANCE
            {
                return None;
            }
            ...
            if crossings[..crossing_count]
                .iter()
                .any(|crossing| crossing.key == point_key)
            {
                break;
            }
            let crossing = crossings.get_mut(crossing_count)?;
            *crossing = Crossing {
                subject_edge,
                clip_edge,
                subject_parameter,
        ...
        if cross(clip_vector, subject_vector) > 0.0 {
            if cross(
                clip_vector,
                subtract(subject[subject_index], clip_previous),
            ) >= 0.0
            {
            ...
            subject_vector,
            subtract(clip[clip_index], subject_previous),
        ) >= 0.0
        {
            subject_previous = subject[subject_index];
            subject_index = (subject_index + 1) % subject.len();
...
fn position_after(first: Position, second: Position) -> bool {
    first.edge > second.edge
        || (first.edge == second.edge && first.parameter > second.parameter)
}
...
fn build_arcs(
    ...
    let mut positions = [
        crossing_position(crossings[0], 0, subject),
        crossing_position(crossings[1], 1, subject),
    ];
    ...
    let inside_other =
        forward_arc_enters_other(positions[0], path, other, crossings, subject)?;
    Some([
        Arc {
            subject,
            start: positions[0],
...
fn forward_arc_enters_other(
    ...
    let other_edge = if subject {
        crossing.clip_edge
    } else {
        crossing.subject_edge
    };
    let path_vector = subtract(path[(start.edge + 1) % path.len()], path[start.edge]);
    let other_vector = subtract(
...
fn select_chain(
    ...
    *chain = Chain {
        arc,
        reversed,
        start_crossing: if reversed {
            arc.end.crossing
        } else {
            arc.start.crossing
        },
        end_crossing: if reversed {
            arc.start.crossing
        } else {
            arc.end.crossing
        },
...
fn stitch_chains(
    ...
    for (index, chain) in chains.iter().enumerate() {
        ...
        let insertion = candidates[..candidate_count]
            ...
            .position(|candidate| {
                compare_angle(
                    first_vector(chains[*candidate], subject, clip, crossings),
                    reverse,
                ) != Ordering::Less
            })
    ...
    for start in 0..chains.len() {
        ...
        if path.len() < 3 || area2(&path) <= f64::EPSILON {
            return None;
        }
...
fn reverse_incoming_vector(
    ...
    if chain.reversed {
        subtract(
            next_forward_point(chain.arc, path, crossings),
            crossings[chain.arc.start_crossing].point,
        )
    } else {
        subtract(
            previous_forward_point(chain.arc, path, crossings),
            crossings[chain.arc.end_crossing].point,
        )
    }
}
...
fn forward_vertex_count(arc: Arc, path_len: usize) -> usize {
    if arc.start.edge == arc.end.edge {
        if arc.wraps {
            path_len
        } else {
            0
        }
    } else if arc.end.edge > arc.start.edge {
        arc.end.edge - arc.start.edge
    } else {
        path_len - arc.start.edge + arc.end.edge
    }
}
...
fn segment_intersection(
    ...
    if robust_cross_order(first_vector, second_vector).is_none() {
        return None;
    }
...
fn collinear_edges_overlap(
    ...
    .is_none()
        && robust_cross_order(
            subtract(second_start, first_start),
            subtract(first_end, first_start),
        )
        .is_none()
...
fn compare_angle(first: PointD, second: PointD) -> Ordering {
    ...
    let second_upper =
        second.y > 0.0 || (second.y.abs() <= f64::EPSILON && second.x >= 0.0);
