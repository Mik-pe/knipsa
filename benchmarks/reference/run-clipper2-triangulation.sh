...
binary=$build_root/knipsa-clipper2-triangulation-benchmark
clipper_cpp=$clipper_root/CPP
mkdir -p "$build_root"
${CXX:-clang++} -O3 -DNDEBUG -std=c++17 -Wall -Wextra -Wpedantic \
  -I"$clipper_cpp/Clipper2Lib/include" \
  "$repo_root/benchmarks/reference/clipper2/triangulation-adapter.cpp" \
...
python3 "$repo_root/benchmarks/reference/clipper2/encode-triangulation-workload.py" \
  "${1:-$repo_root/benchmarks/triangulation-workloads.json}" | "$binary"
